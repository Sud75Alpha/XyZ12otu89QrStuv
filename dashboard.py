"""
GOLD/DXY PRO — Dashboard v10 FINAL
Sidebar native · Titre · Timeframe · Tabs complets
"""
...
# ─── SESSION STATE ────────────────────────────────────────────────────────────
_INIT: Dict = {
    "tick":0,"ws_connected":False,"ws_queue":None,"ws_thread":None,"ws_stop":None,
...
# ─── WEBSOCKET THREAD ─────────────────────────────────────────────────────────
def _ws_fn(q,stop):
    if not HAS_WS: q.put({"type":"_ws_status","connected":False}); return
    delay=2
    while not stop.is_set():
        try:
            w=_ws.create_connection(WS_URL,timeout=10)
            q.put({"type":"_ws_status","connected":True}); delay=2
            while not stop.is_set():
                try:
        ...
        except: q.put({"type":"_ws_status","connected":False})
...
def _apply(d):
    for k in ["gold_price","dxy_price","gold_bid","gold_ask","gold_change","gold_pct",
               ...
               "winrate","wins","losses","bot_status","mt5_connected","gold_symbol","last_update","zones"]:
        if k in d: ss[k]=d[k]
...
def _process_ws():
    ...
    while not q.empty() and n<60:
        ...
        if   t=="_ws_status": ss.ws_connected=msg.get("connected",False)
            ...
            ss.gold_price=msg.get("gold",ss.gold_price); ss.dxy_price=msg.get("dxy",ss.dxy_price)
            ss.gold_bid=msg.get("gold_bid",ss.gold_bid); ss.gold_ask=msg.get("gold_ask",ss.gold_ask)
            ss.gold_change=msg.get("gold_change",0.0); ss.gold_pct=msg.get("gold_pct",0.0)
            ss.dxy_change=msg.get("dxy_change",0.0); ss.correlation=msg.get("corr",ss.correlation)
        elif t=="signal":
            if "signal" in msg: ss.signal=msg["signal"]
            if "mtf"    in msg: ss.mtf=msg["mtf"]
            if "stats"  in msg:
                ss.winrate=msg["stats"].get("winrate",ss.winrate)
                ss.wins=msg["stats"].get("wins",ss.wins); ss.losses=msg["stats"].get("losses",ss.losses)
        elif t=="ohlcv":
...
def _simulate():
    ...
    ss.gold_change=round(np.random.normal(0,.7),2); ss.gold_pct=round(np.random.normal(0,.035),3)
    ss.dxy_change=round(np.random.normal(0,.015),4)
    ss.correlation=max(-1,min(1,ss.correlation+np.random.normal(0,.008)))
    ss.gold_bid=round(ss.gold_price-.15,2); ss.gold_ask=round(ss.gold_price+.15,2)
...
def _sim_ohlcv(n,mins,sym):
    ...
    np.random.seed(int(time.time()*2)%10000); cl[-1]*=(1+np.random.normal(0,vol*.3))
    out,now=[],datetime.now()
    for i in range(n):
        t=now-timedelta(minutes=mins*(n-1-i)); o=cl[i-1] if i>0 else cl[i]; c=cl[i]
        r=abs(c-o)*(1+abs(np.random.normal(0,.4))); h=max(o,c)+r*.35; l=min(o,c)-r*.35
        out.append({"time":t.isoformat(),"open":round(o,5),"high":round(h,5),
                     "low":round(max(l,.1),5),"close":round(c,5),"volume":int(np.random.exponential(2000))})
    return out
...
def _rolling_corr(gold_c,dxy_c,window=50):
    ...
    t=[c["time"]  for c in gold_c[-n:]]; out=[]
...
def build_candle(candles,symbol,color,tf,signal=None,zones=None,show_zones=True):
   ...
   # FVG haute qualité depuis fvg_engine
    from fvg_engine import detect_quality_fvg, draw_fvg_on_figure
    if show_zones and not df.empty:
        fvgs = detect_quality_fvg(df, use_trend_filter=True, max_fvg=5)
        fig  = draw_fvg_on_figure(fig, fvgs, row=1, col=1)
...
# ─── FORCE SIDEBAR OUVERTE — JS + CSS combinés ───────────────────────────────
st.markdown("""
<style>
/* Force sidebar visible — tous les sélecteurs Streamlit Cloud */
...
/* Cacher le bouton collapse et la flèche */
...
<script>
// Force sidebar ouverte via JS — s'exécute après le rendu
(function forceSidebar() {
    function fix() {
        ...
        // Cacher les boutons collapse
        var btns = document.querySelectorAll(
            '[data-testid="stSidebarCollapseButton"], [data-testid="collapsedControl"], button[aria-label="Close sidebar"], button[aria-label="Open sidebar"]'
        );
...
# Titre
ws_col="#a78bfa" if ws_ok else "#f7b529"; dh="dp" if ws_ok else "dr"
...
<div style="display:flex;align-items:flex-start;justify-content:space-between;
    ...
    <div>
        ...
        <div style="font-size:.6rem;color:#6b7a94;margin-top:2px;">
            <span class="{dh}"></span><span style="color:{ws_col};">{'WS Live' if ws_ok else 'HTTP'}</span>
...
# Tabs
tab1,tab2,tab3,tab4,tab5=st.tabs(["📊 Graphiques","🎯 Signal & Zones","🔀 Multi-TF","📋 Logs Bot","📜 Historique"])

with tab1:
    g1,g2=st.columns(2)
    with g1: _plt(build_candle(gold_c,"XAUUSD",C["gold"],tf,signal=signal,zones=zones,show_zones=ss.show_zones),key="gold_chart")
    with g2: _plt(build_candle(dxy_c,"DXY",C["dxy"],tf),key="dxy_chart")
    _plt(build_corr_chart(corr_data),key="corr_chart")
    if corr_data:
        lc=corr_data[-1]["corr"]; zc="#00d4aa" if lc<-.6 else ("#f7b529" if lc<-.4 else "#ff4d6a")
        zt="Zone signal active" if lc<-.6 else ("Modérée" if lc<-.4 else "Hors zone signal")
        st.markdown(f'<div style="font-size:.57rem;color:{zc};margin-top:1px;">Corr: <b>{lc:+.4f}</b> · {zt} · fenêtre 50 bougies</div>',unsafe_allow_html=True)
...
with tab2:
    ...
    with s2:
        dir2=signal.get("direction","WAIT"); ant2=signal.get("anticipation") or ""
        ...
        if dir2 in ("BUY","SELL"):
            entry=signal.get("entry",0);tp=signal.get("tp",0);sl=signal.get("sl",0);rr=signal.get("rr",0)
            st.markdown(f"""
            <div class="card card-gld">
    ...
    with s3:
        sup=zones.get("support",0);res=zones.get("resistance",0);atr=zones.get("atr",0);ff=zones.get("fvg_filter",0)
...
with tab3:
    ...
    for col,tf_n in zip(mc,["H1","M15","M5"]):
        d=mtf.get(tf_n,{}); sig=d.get("signal","WAIT"); cr=d.get("corr",0.); tr=d.get("trend","—"); at=d.get("anticipation") or ""
...
with tab4:
    ...
    st.markdown(f'<div style="margin-top:5px;font-size:.6rem;color:{bot_col};">Bot:{ss.bot_status} · MT5:{"✅" if mt5_ok else "⚠️"} · WS:{"🟣" if ws_ok else "🔴"} · {str(ss.last_update)[:19]}</div>',unsafe_allow_html=True)
...
with tab5:
    ...
    if ss.signals:
        ...
        def _sty(v):
            return {"BUY":"color:#00d4aa;font-weight:700","SELL":"color:#ff4d6a;font-weight:700","WIN":"color:#00d4aa","LOSS":"color:#ff4d6a"}.get(str(v),"color:#6b7a94")
        try:
        ...
        st.markdown('<div style="text-align:center;padding:35px;color:#2e3a4e;font-size:.7rem;">Aucun signal enregistré.<br>Lance le bot + api_server.py</div>',unsafe_allow_html=True)

st.markdown(f'<div style="text-align:center;padding:4px 0 2px;font-size:.48rem;color:#1a2234;border-top:1px solid rgba(255,255,255,.04);margin-top:4px;">Gold/DXY Pro v10 · {"🟣WS" if ws_ok else "🟡HTTP"} · Tick#{ss.tick} · {REFRESH_S}s</div>',unsafe_allow_html=True)
