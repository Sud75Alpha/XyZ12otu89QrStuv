"""
╔══════════════════════════════════════════════════════════════════════════════╗
║         API SERVER v3 — GOLD/DXY BOT  ·  HTTP + WebSocket                  ║
║                                                                              ║
...
class GlobalState:
    def __init__(self):
        ...
        self.bot_logs.append({"time": datetime.now().strftime("%H:%M:%S"),
                               "level": "INFO", "msg": "API Server v3 WebSocket démarré"})
...
def compute_zones(candles: List[Dict]) -> Dict:
    ...
    # ATR — sert à filtrer les FVG trop petits
...
def mt5_data_thread():
    ...
    while True:
        try:
            ...
            with STATE._lock:
                STATE.gold_prev  = STATE.gold_price or gold_p
                STATE.dxy_prev   = STATE.dxy_price  or dxy_p
                STATE.gold_price = gold_p; STATE.dxy_price  = dxy_p
                STATE.gold_bid   = gold_b; STATE.gold_ask   = gold_a
                STATE.last_update = datetime.now().isoformat()

            # OHLCV toutes les 5 itérations (~2.5s)
            ohlcv_tick += 1
            if ohlcv_tick >= 5:
                ohlcv_tick = 0
                for tf_name, tf_min in TF_MAP.items():
                    ...
                    if MT5_AVAILABLE and STATE.mt5_connected and STATE.gold_symbol:
                        rates = mt5.copy_rates_from_pos(STATE.gold_symbol, MT5_TF[tf_name], 0, n_bars)
            ...
            if gc:
                ...
                with STATE._lock:
                    ...
                    STATE.current_signal.update({
                        "direction": sig["direction"], "anticipation": sig.get("anticipation"),
                        "confidence": sig["confidence"], "corr": round(corr,4),
                        "gold_price": round(gold_p,2), "dxy_price": round(dxy_p,3),
                        "time": datetime.now().isoformat(),
                    })
...
app = FastAPI(title="Gold/DXY API v3", version="3.0.0")
...
# ── HTTP (fallback + push bot) ─────────────────────────────────────────────────

@app.get("/")
def root():
    return {"service": "Gold/DXY API v3", "ws_clients": ws_manager.count,
...
class SignalPayload(BaseModel):
    direction: str; tf: str; entry: float; tp: float; sl: float; rr: float
    sl_source: str = "ATR"; corr: float = 0.0; confidence: int = 0
    anticipation: Optional[str] = None; pipeline_state: str = "IDLE"; result: str = "OPEN"

@app.post("/api/signal/push")
async def push_signal(payload: SignalPayload, request: Request):
    _auth(request)
    data = payload.dict(); data["time"] = datetime.now().isoformat()
...
async def startup_event():
    ...
    log.info(f"API v3 | HTTP+WS :{API_PORT} | ws://localhost:{API_PORT}/ws?api_key={API_KEY}")
